...
class PypersMulti:
    ...
    def __init__(self,nrows=1,ncols=2,cbloc="right",cbmode="single",landscape=True):
...
def plotxy(fig,ax,x,y,log=False):
    ...
    def decor(fig,ax,xlim=(1.0, 3.0),ylim=(1.0,3.0),xlabel=r"$x$",ylabel=r"$y$",**kw):
        kw = { minticks_on : True,
               labelsize   : 20,
               lwidth      : 1.5
        }
        ...
        if minticks_on:
            ax.minorticks_on()
            ax.tick_params(axis='both', which='minor', length=3, width=lwidth)
        ax.tick_params(axis='both', which='major', length=6, width=lwidth, labelsize=16)
        ...
        for axis in ['top','bottom','left','right']:
            ax.spines[axis].set_linewidth(lwidth)
